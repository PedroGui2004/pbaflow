(function () {
  ...
  function normalizeRepairTimers() {
    repairRows.forEach(function (repair) {
      ...
      if (repair.status === "active" && !repair.timerPaused && !repair.startedAt) repair.startedAt = nowMs();
  ...
  function handleAction(action, element) {
    ...
    if (action === "start-repair" || action === "resume-repair") { var startRow = repairRows.find(function (row) { return row.id === Number(element.dataset.id); }); if (startRow) { startRow.status = "active"; startRow.timerPaused = false; startRow.startedAt = nowMs(); startRow.waitingReason = ""; startRow.waitingNotes = ""; startRow.pauseReason = ""; saveOperations(); state.repairTab = "active"; render(); showToast("Reparo em andamento no nome de " + startRow.tech + "."); } }
    ...
    if (action === "save-repair-time") { var correctedRow = repairRows.find(function (row) { return row.id === Number(element.dataset.id); }); var correctionReason = $("#correct-justification").value.trim(); if (!correctionReason) { showToast("A justificativa é obrigatória."); return; } if (correctedRow) { var correctedSeconds = Math.max(0,Number($("#correct-hours").value || 0) * 3600 + Number($("#correct-minutes").value || 0) * 60 + Number($("#correct-seconds").value || 0)); correctedRow.elapsedSeconds = correctedSeconds; if (correctedRow.status === "active" && !correctedRow.timerPaused) correctedRow.startedAt = nowMs(); correctedRow.timeCorrections = correctedRow.timeCorrections || []; correctedRow.timeCorrections.push({at:new Date().toISOString(),by:state.user,seconds:correctedSeconds,justification:correctionReason}); correctedRow.notes = [correctedRow.notes,"Correção de tempo por " + state.user + ": " + correctionReason].filter(Boolean).join(" · "); saveOperations(); $("#modal").close(); render(); showToast("Cronômetro corrigido com justificativa registrada."); } }
    ...
    if (action === "start-channel") {
      ...
      kvmSessions.push({ key:state.selectedChannel, op:startOp, serial:startSerial, tech:startTech, system:plannedStart && plannedStart.system ? plannedStart.system : machineSystem(startSerial), status:"testing", elapsedSeconds:0, startedAt:nowMs(), failures:0, connection:channelType(Number(channelMatch[1]),Number(channelMatch[2])) });
